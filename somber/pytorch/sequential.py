...
class Merging(Sequential):
    ...
    def _get_bmus(self, x, **kwargs):
        ...
        distances_y = self.distance_function(kwargs['context'], self.context_weights)

        # BMU is based on a weighted addition of current and previous activation.
        activations = t.squeeze((t.mul(distances_x, 1 - self.alpha) + t.mul(distances_y, self.alpha)))
