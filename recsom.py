...
class RECsom(Som):
    ...
    def __init__(self, width, height, dim, learning_rate, alpha, beta, lrfunc=expo, nbfunc=expo):
        ...
        self.context_weights = np.copy(self.weights)
    ...
    def epoch_step(self, X, map_radius, learning_rate, batch_size):
        ...
        for index in progressbar(range(num_batches), idx_interval=1, mult=batch_size):
            ...
            previous_activations = np.zeros((current.shape[0], self.map_dim, self.data_dim))
            ...
            for idx in range(current.shape[1]):
                ...
                bmu_theta, x_activations, y_activations = self._get_bmus(column, y=previous_activations)

                previous_activations = (self.alpha * x_activations) + (self.beta * y_activations)
    ...
    def _get_bmus(self, x, **kwargs):
        ...
        distances = np.exp(-(self.alpha * distances_x) - self.beta * distances_y)
    ...
    def predict(self, X):
        ...
        y_activations = np.zeros((len(X), self.map_dim, self.data_dim))
        ...
        for idx in range(X.shape[1]):
            ...
            column = X[:, idx, :]
            bmus, _, y_activations = self._get_bmus(column, y=y_activations)
