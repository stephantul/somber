...
class Som(object):
    ...
    def fit(self, num_epochs, data, return_history=0):
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % return_history == 0:
                history.append(self.map_weights())
            ...
            for vector, bmu_idx in zip(data, coords):
                ...
                self._update(vector, indices, influence, learning_rate)
    ...
    def predict(self, x):
        ...
        return self._indices_to_coords(self._get_bmus(x, self.weights))
    ...
    def _get_bmus(x, weights):
        ...
        return [np.argmin(np.array((weights - vector) ** 2).sum(axis=1)) for vector in x]
...
if __name__ == "__main__":
    ...
    start = time.time()
    s = Som(50, 50, 3, 0.3)
    history = s.fit(400, colors, return_history=10)
