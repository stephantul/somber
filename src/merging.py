...
class MergingGas(Ng):
    ...
    def __init__(self, map_dim, dim, learning_rate, alpha, beta, sigma=None, lrfunc=expo, nbfunc=expo):
        ...
        super().__init__(map_dim, dim, learning_rate, sigma, lrfunc, nbfunc)

        self.alpha = alpha
        self.beta = beta
        self.context_weights = np.random.uniform(0.0, 0.1, self.weights.shape)
        self.entropy = 0

    def train(self, X, num_effective_epochs=10, fix_training=0.5, use_entropy=True):
        ...
        for idx, x in enumerate(progressbar(X)):
            ...
            if idx % epoch_counter == 0:
                ...
                if epoch > fixed_epoch and use_entropy:
                    update = self._entropy(bmu_entropy, prev_update)
                    self.alpha += update
                    self.alpha = np.clip(self.alpha, 0.0, 1.0)
                    logger.info("Alpha: {0}".format(self.alpha))
